...
class HashTable:
    ...
    def hashing(self, board):
        ...
        hash_key=format(hashing_value, '064b')[0:56]
        hash_key=int(hash_key, 2)
        core_dest=format(hashing_value, '064b')[-8:]
...
def TDS_UCT(chem_model):
    ...
    while time.time()-start_time<=600:
       # comm.bsend(np.asarray([n.state,n.reward,n.wins,n.visits,n.num_thread_visited]),dest=dest,tag=0
       # comm.bsend(np.asarray([['&'],None,0,0,0]),dest=rootdest,tag=0)
        ...
        noniprobe_stime=time.time()
       # comm.bsend(np.asarray([['&'],None,0,0,0]),dest=rootdest,tag=0)

        if ret==True:
            busy_stime=time.time()
            ...
            node.wins=message[2]
            node.visits=message[3]
            node.num_visited_thread=message[4]
                       ...
                       #     n.num_thread_visited]),dest=dest,tag=0)
                        ...
                        if len(node.state)<81:
                            if node.state[-1]!='\n':
                                ...
                                #comm.bsend(np.asarray([root,None,0,0,0]),dest=rootdest,tag=0)
                                send_stime=time.time()
            ...
            if cur_status.Get_tag()==1:
                ...
                if local_node.state==['&']:
                    ...
                    communication_time+=send_ftime
                    com_time.append(send_ftime)
            busy_ftime=time.time()-busy_stime
            busy_time+=busy_ftime
            bu_time.append(busy_ftime)

        noniprobe_ftime=time.time()-noniprobe_stime
        noniprobe_time+=noniprobe_ftime



    run_time.append(time.time()-start_time)
    nonip_time.append(noniprobe_time)
    exptime.append(expansion_time)
    simtime.append(simulation_time)
    comtime.append(communication_time)
    busytime.append(busy_time)
    ip_time.append(iprobe_time)

    write_to_csv(ip_time,'iprobetime'+str(rank))
    write_to_csv(nonip_time,'noniprobetime'+str(rank))
    write_to_csv(run_time,'runtime'+str(rank))
    write_to_csv(busytime,'busytime'+str(rank))
    write_to_csv(bu_time,'busy_time'+str(rank))
    ...
    write_to_csv(com_time,'com_time'+str(rank))
