...
def simulation(chem_model,state,rank,gauid):
    ...
    if m!=None:
        ...
        if stable==1.0:
            try:
                SDFinput = 'OUTPUT/wavelength_dmcts_CheckMolopt'+str(ind)+'.sdf'
                ...
                if hsm.search_table(message[0])==None: #if node is not in the hash table
                    ...
                    if node.state==['&']:
                        ...
                        if len(node.state)<41:
                            ...
                            write_to_csv(allscore,'OUTPUT/wavelength_dmcts_scoreForProcess'+str(rank))
                            write_to_csv(allmol,'OUTPUT/wavelength_dmcts_generatedMoleculesForProcess'+str(rank))
