...
def selection(node):
    ...
    for i in range(len(node.childNodes)):
        ucb.append((node.childNodes[i].wins +
                   ...
                   (node.childNodes[i].visits +
                    node.childNodes[i].num_thread_visited) +
                   1.0 *
                   sqrt(2 *
                        log(node.visits +
...
def compare_ucb(pnode):
    for path_ucb in pnode.path_ucb:
        ...
        for i in range(len(path_ucb) - 1):
            ind = path_ucb[0][3]
            ucb.append((path_ucb[i + 1][0] + 0) / (path_ucb[i + 1][1] + path_ucb[i + 1][2]) + 1.0 * sqrt(
                2 * log(path_ucb[0][1] + path_ucb[0][2]) / (path_ucb[i + 1][1] + path_ucb[i + 1][2])))
...
def TDS_UCT(chem_model):
    ...
    for i in range(3):
        comm.bsend(np.asarray([['&'], None, 0, 0, 0, []]),
                   dest=rootdest, tag=0)
    ...
    while time.time() - start_time <= 600:
        ...
        if ret:
            ...
            if cur_status.Get_tag() == 0:
                if hsm.search_table(
                        message[0]) is None:  # if node is not in the hash table
                    ...
                    if node.state == ['&']:
                        ...
                        if len(node.state) < 81:
                            ...
                            node = update_local_node(
                                node, score)  # backpropagation on local memory
                            hsm.insert(Item(node.state, node))
                            ...
                            if node.state[-1] != '\n':
                                if node.expanded_nodes != []:
                                    ...
                                    if node.check_childnode == []:
                                        ...
                                        ind, childnode, node = selection(node)
                                        hsm.insert(Item(node.state, node))
                                        ucb_table = update_selection_ucbtable(
                                            node, ind)
                                        _, dest = hsm.hashing(childnode.state)
                                        send_stime = time.time()
                                        comm.bsend(
    ...
    write_to_csv(allscore, 'OUTPUT/logp_dmcts_scoreForProcess' + str(rank))
    write_to_csv(
        allmol,
        'OUTPUT/logp_dmcts_generatedMoleculesForProcess' +
        str(rank))
