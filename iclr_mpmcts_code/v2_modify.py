...
def TDS_UCT(chem_model,num_cores):
    ...
    while True:
        ...
        if ret==True:
            ...
            #print ("checks:",checks)
            """
            if tag=search, then perform selection, expansion and simulation.
            checks is the flag: 0 indicate the rank is doing simulation, 1 indicates the rank
            is not doing simulation.
            
            """
            if checks==1:
                #checks=1
                listget,cur_status=q.get()
                state_in_queue1=listget[0]
                if cur_status.Get_tag()==0:
                    if hs.get(make_string(message[0]))==None:
                        print ("growing the tree in rank"+" "+str(rank)+" "+"Checks"+" "+str(checks))
                        ...
                        ###### starting a child thread to perform simulation
                        start_time=time.time()
                        sim_thread=Thread(target=simulation,args=(chem_model,state_in_queue1,q1))
                        sim_thread.start()
                        if q1.qsize()!=0:
                            checks=0
                            score, state_in_queue=q1.get()
                            finish_time=time.time()-start_time
                            #print ("state_in_queue:",state_in_queue)
                            print ("rank"+" "+str(rank)+" "+"finished simulation with"+" "+str(score)+" "+
                                "sim time"+" "+str(finish_time))
                            #node_in_queue=update_report(node_in_queue,score)
                            if hs.search_table(make_string(state_in_queue))!=None:
                                node=hs.search_table(make_string(state_in_queue))
                                node=update_reward(node,score)
                                hs.insert(Item(make_string(state_in_queue),node))
                                if state_in_queue==["&"]:
                                    comm.bsend([state_in_queue,score,state_in_queue[-1]],
                                    dest=hs.hashing(make_string(state_in_queue)),tag=0)
                                else:
                                    comm.bsend([state_in_queue[0:-1],score,state_in_queue[-1]],
                                    dest=hs.hashing(make_string(state_in_queue[0:-1])),tag=1)
                            else:
                                print ("error happens")
                    else:
                        print ("updating the tree in rank"+" "+str(rank)+" "+ "checks"+" "+str(checks))
                        node=hs.search_table(make_string(message[0]))
                        node=update_tree_grow(node)
                        childnode,node=selection(node)
                        hs.insert(Item(make_string(node.state),node))
                        #print ("updating the tree with child UCB:",node.childucb)
                        comm.bsend([childnode.state,message[1],childnode.state[-1]],
                        ...
                        if q1.qsize()!=0:
                            checks=0
                            score, state_in_queue=q1.get()
                            #finish_time_update=time.time()-start_time_update
                            #print ("check q1:",state_in_queue)
                            print ("rank"+" "+str(rank)+" "+"finished simulation with"+" "+str(score))#+" "+
                               #"sim time"+" "+str(finish_time_update) )
                            #node_in_queue=update_report(node_in_queue,score)
                            if hs.search_table(make_string(state_in_queue))!=None:
                                node=hs.search_table(make_string(state_in_queue))
                                node=update_reward(node,score)
                    ...
                    if node.state==['&']:
                        ...
                        hs.insert(Item(make_string(node.state),node))
                        comm.bsend([node.state,message[1],message[2]],
                                dest=hs.hashing(make_string(node.state)),tag=0)
                    else:
                        node=update_report_reward(node,message[1],message[2])
                        #print ("check correctness:",node.state)
                        hs.insert(Item(make_string(node.state),node))
                        comm.bsend([node.state[0:-1],message[1],message[2]],
