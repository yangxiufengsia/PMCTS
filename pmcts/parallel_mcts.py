...
backtrack_mpmcts_top2, compare_ucb_tdsdfuct_all, compare_ucb_tdsdfuct_top2, compare_ucb_mpmcts_all, compare_ucb_mpmcts_top2, update_selection_ucbtable_tdsdfuct_all,
update_selection_ucbtable_tdsdfuct_top2,update_selection_ucbtable_mpmcts_all, update_selection_ucbtable_mpmcts_top2,
...
class p_mcts:
    ...
    """
    def TDS_df_UCT_top2(chem_model, hsm, property, comm):
        #comm.barrier()
        rank = comm.Get_rank()
            ...
            if jobq_non_empty:
                ...
                if tag == JobType.SEARCH.value:
                    if hsm.search_table(message[0]) == None:
                        ...
                        if node.state == ['&']:
                            ...
                            if len(node.state) < node.max_len:
                                ...
                                ####update infor table
                                info_table=backtrack_tdsdfuct_top2(info_table,score)
                                hsm.insert(Item(node.state, node))
                        ...
                        if node.state == ['&']:
                            ...
                            if node.expanded_nodes != []:
                                ...
                                hsm.insert(Item(node.state, node))
                                info_table = update_selection_ucbtable_tdsdfuct_top2(info_table, node, ind)
                    ...
                    if local_node.state == ['&']:
                        ...
                        back_flag = compare_ucb_tdsdfuct_top2(info_table,local_node)
                        hsm.insert(Item(local_node.state, local_node))
                        if back_flag == 1:
    ...
    def TDS_df_UCT_all(chem_model, hsm, property, comm):
        #comm.barrier()
        rank = comm.Get_rank()
        nprocs = comm.Get_size()
        status = MPI.Status()
            ...
            if jobq_non_empty:
                ...
                if tag == JobType.SEARCH.value:
                    if hsm.search_table(message[0]) == None:
                        ...
                        if node.state == ['&']:
                            ...
                            if len(node.state) < node.max_len:
                                ...
                                node.update_local_node(score)
                                ####update infor table
                                info_table=backtrack_tdsdfuct_all(info_table,score)
                                hsm.insert(Item(node.state, node))
                                _, dest = hsm.hashing(node.state[0:-1])
                                comm.bsend(np.asarray([node.state, node.reward, node.wins, node.visits,
                                                       node.num_thread_visited, info_table]),
                        ...
                        if node.state == ['&']:
                            ...
                            if node.expanded_nodes != []:
                                ...
                                ind, childnode = node.selection()
                                hsm.insert(Item(node.state, node))
                                info_table = update_selection_ucbtable_tdsdfuct_all(info_table,node, ind)
                                #print ("info_table after selection:",info_table)
                                _, dest = hsm.hashing(childnode.state)
                                comm.bsend(np.asarray([childnode.state, childnode.reward, childnode.wins,
                                                       childnode.visits, childnode.num_thread_visited,
                    ...
                    if local_node.state == ['&']:
                        ...
                        local_node.backpropagation(node)
                        #local_node,info_table = backtrack_tdsdf(info_table,local_node, node)
                        back_flag = compare_ucb_tdsuct_all(info_table,local_node)
                        hsm.insert(Item(local_node.state, local_node))
                        if back_flag == 1:
                            _, dest = hsm.hashing(local_node.state[0:-1])
                            comm.bsend(np.asarray([local_node.state, local_node.reward, local_node.wins,
